...
# Accounts that post automated review messages but never vote CR, or
# vote on another label. Together with _CR_BOT_VOTERS and the name
# pattern below they separate human review activity from CI chatter.
_ACTIVITY_BOT_NAMES = frozenset({
    "Lustre RISC-V Builder",
    "Janitor Bot",
    "Gerrit AI review for Lustre",
})
_ACTIVITY_BOT_RE = re.compile(
    r"\b(bot|robot|builder|janitor|jenkins|maloo|autotest|checkpatch)\b",
    re.IGNORECASE,
)


# Gerrit notices from before message tags existed (2019-era changes
# carry them untagged); modern Gerrit tags each "autogenerated:gerrit:*".
_LEGACY_NOTICE_RE = re.compile(
    r"^(Uploaded patch set \d+|Patch Set \d+: Patch Set \d+ was rebased"
    r"|Change has been successfully|Removed the following votes"
    r"|Removed (reviewer|cc)\b|Abandoned(\s|$)|Restored(\s|$))"
)
# Automated notes posted through a person's account by tooling
# (gerrit-cli --prefix '[Marc Bot]', lreview's '[AI review - <model>]'),
# on the first line after Gerrit's "Patch Set N:" header.
...
def _same_person(name: str, owner: str) -> bool:
    """"Patrick Farrell (personal)" is Patrick Farrell's second account."""
    strip = lambda n: re.sub(r"\s*\([^)]*\)$", "", n)
    return strip(name) == strip(owner)


def _is_bot_account(name: str) -> bool:
    return (
        name in _CR_BOT_VOTERS
        or name in _ACTIVITY_BOT_NAMES
        or bool(_ACTIVITY_BOT_RE.search(name))
    )


def _extract_activity(
    messages: list[dict[str, Any]], owner: str,
) -> dict[str, Any]:
    """Human review activity and the abandon time from change messages.

    A review event is a message written by a human who is not the
    change owner. Messages tagged "autogenerated:*" (uploads, rebases,
    hashtag edits, abandon/restore, AI reviews), their untagged legacy
    forms, bot accounts and tool notes posted through a person's
    account are not reviews.

    `abandoned_at` is the date of the last abandon message, cleared by
    a later restore; "" when the change is not currently abandoned by
    message history.
    """
    ...
    for msg in messages:
        ...
        legacy = "" if tag else (_LEGACY_NOTICE_RE.match(text) or [""])[0]
        if tag == "autogenerated:gerrit:abandon" or legacy.startswith("Abandoned"):
            abandoned_at = msg.get("date", "")
            continue
        if tag == "autogenerated:gerrit:restore" or legacy.startswith("Restored"):
            abandoned_at = ""
            continue
        if tag.startswith("autogenerated:") or legacy:
            continue
        name = (msg.get("author") or {}).get("name", "")
        if (not name or _same_person(name, owner) or _is_bot_account(name)
                or _TOOL_PREFIX_RE.match(text)):
            continue
        t = gerrit_epoch(msg.get("date", ""))
        if not t:
            continue
        review_times.append(t)
        reviewers[name] = reviewers.get(name, 0) + 1
    review_times.sort()
    return {
