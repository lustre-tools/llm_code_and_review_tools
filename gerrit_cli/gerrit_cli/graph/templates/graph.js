    ...
    panel.innerHTML = `
        <div class="field">
            ...
            <div class="fv">
                ...
                ${node.current_commit ? `<button onclick="navigator.clipboard.writeText('${node.current_commit}');const o=this.textContent;this.textContent='\u2713 copied';setTimeout(()=>this.textContent=o,1200)" style="cursor:pointer;font-family:monospace;font-size:12px;background:none;border:1px solid var(--border);border-radius:4px;padding:1px 6px;color:var(--accent);margin-left:6px" title="Click to copy full SHA: ${node.current_commit}">${node.current_commit.substring(0, 7)}</button>` : ''}
            </div>
        </div>
        <div class="field">
