...
class TestCmdReviewPostComments:
    ...
    def test_review_post_comments_nested_format_with_prefix(self, tmp_path):
        ...
        review_file.write_text(json.dumps({
            ...
            "comments": {
                "a.c": [{"line": 5, "message": "fix", "unresolved": True}],
                "b.c": [{"line": 7, "message": "typo"}],
            },
        }))
        ...
        with patch('gerrit_cli.cli.CodeReviewer') as MockReviewer:
            ...
            mock_post_result = MagicMock()
            mock_post_result.success = True
            mock_post_result.comments_posted = 2
            ...
            call_kwargs = MockReviewer.return_value.post_review.call_args[1]
            assert call_kwargs['comments'] == [
                {"path": "a.c", "line": 5, "message": "fix", "unresolved": True},
                {"path": "b.c", "line": 7, "message": "typo"},
            ]
            assert call_kwargs['message'] == "Overall message"
            assert call_kwargs['prefix'] == "[Marc Bot]"
            assert call_kwargs['tag'] == "autogenerated:ai-review"
    ...
    def test_review_post_comments_dry_run(self, tmp_path, capsys):
        ...
        review_file = tmp_path / "review.json"
        review_file.write_text(json.dumps({
            "comments": {"a.c": [{"line": 5, "message": "fix"}]},
        }))
        ...
        with patch('gerrit_cli.cli.CodeReviewer') as MockReviewer:
            ...
            would_post = output["would_post"]
            assert would_post["comment_count"] == 1
            assert would_post["prefix"] == "[Marc Bot]"
            assert would_post["comments"][0]["path"] == "a.c"
            # Preview shows the final text exactly as it would be posted
            assert would_post["comments"][0]["message"] == (
                "**[Marc Bot]**\n\nfix")
    ...
    def test_review_post_comments_cli_fallbacks(self, tmp_path):
        ...
        review_file = tmp_path / "review.json"
        review_file.write_text(json.dumps({"comments": []}))
        ...
        with patch('gerrit_cli.cli.CodeReviewer') as MockReviewer:
            ...
            call_kwargs = MockReviewer.return_value.post_review.call_args[1]
            assert call_kwargs['message'] == "CLI message"
            assert call_kwargs['vote'] == 1
            assert call_kwargs['tag'] == "cli-tag"

    def test_review_post_comments_cli_overrides_file(self, tmp_path):
        """Explicit CLI --message/--vote/--tag override the file values.
    ...
    def test_review_post_comments_file_vote_zero_used(self, tmp_path):
        ...
        review_file = tmp_path / "review.json"
        review_file.write_text(json.dumps({"comments": [], "vote": 0}))
