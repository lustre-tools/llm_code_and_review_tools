...
def add_review_parser(subparsers):
    ...
    parser.add_argument(
        "--prefix",
        metavar="TEXT",
        help="Prefix prepended to the review message and every comment, "
             "e.g. '[Marc Bot]' (with --post-comments)",
    )
    parser.add_argument(
        "--tag",
        metavar="TAG",
        help="Gerrit review tag, e.g. 'autogenerated:ai-review'; "
             "overrides the 'tag' field in the JSON file (with --post-comments)",
    )
    parser.add_argument(
        "--dry-run", "-n",
        action="store_true",
