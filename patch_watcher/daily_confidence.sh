#!/bin/bash
# daily_confidence.sh — Daily confidence report for the patch watcher.
#
...
HTML_FILE="/tmp/patch_watcher_daily.html"
...
LOG_DIR = os.path.expanduser("~/.patch_watcher")
...
# --- Build subject ---
subject = (
    f"[Patch Watcher] Daily: {health_grade} — "
...
padding-bottom: 8px; margin-bottom: 16px;">
  Patch Watcher — Daily Confidence Report
</h2>
...
font-size: 11px; color: #888;">
  <strong>Patch Watcher Daily Confidence Report</strong><br>
...
with open("/tmp/patch_watcher_daily_subject.txt", "w") as f:
    f.write(subject)
...
SUBJECT="$(cat /tmp/patch_watcher_daily_subject.txt)"
...
# Cleanup
rm -f "$HTML_FILE" /tmp/patch_watcher_daily_subject.txt
