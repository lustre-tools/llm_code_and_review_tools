"""Small, side-effect-free renderers for deterministic retest controls."""
...
def render_global_retest_status(
    *, execution_enabled: bool, csrf_token: str, recent_summary: str = "",
    ...
    `nested` renders it as a stage inside a larger automation card rather than
    a card of its own. It used to be one of two separate top-level cards --
    "Automatic actions" and "Autonomous lanes" -- which read as two unrelated
    features when they are in fact two gates over the same thing.
    """
    ...
    return (
        f"<section class='{'' if nested else 'card '}retest-global' "
        "aria-labelledby='retest-global-title'>"
        f"<div class='section-title'><div><{heading} id='retest-global-title'>"
        f"Saved patch policies</{heading}>"
        "<p class='sub'>Test, build, and review policies saved on individual "
        "patches run only while this gate is on.</p></div>"
...
def render_enable_confirmation(*, csrf_token: str) -> str:
    return (
        ...
        "Claude with a host shell and real service credentials, may create LTVM guests, "
        "and make their own Gerrit writes.</p>"
