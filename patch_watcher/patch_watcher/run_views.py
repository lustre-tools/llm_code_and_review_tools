"""Accessible HTML views for managed agent runs.

Inputs may be mappings, dataclasses, or other attribute objects.  The module
only renders controls: the controller owns CSRF, idempotency, and
confirmation-token validation.  GET links only open detail/confirmation
views; every mutation uses POST.
"""
...
# The capability profile `claude_runner.ReadOnlyRunSpec` accepts that grants
# the agent a real shell, file writes, and the operator's service credentials.
# The only other one it accepts -- "read_only" -- gets Read/Glob/Grep under
# `--safe-mode --restricted` with those credentials scrubbed. The intermediate
# `source_edit` and `source_edit_ltvm` profiles were removed with the MCP
# server the second required; naming them here would suggest a page could
# still receive one.
FULL_CAPABILITY_PROFILES = {"full"}
# .capitalize() lowercases everything after the first letter, so an event type
# such as "ltvm_cleanup_abandoned" rendered as "Ltvm cleanup abandoned". Spell
# the initialism-bearing labels out instead of deriving them, exactly as the
# watch-state chips already do.
HUMAN_LABELS = {
    "ci_failed": "CI failed",
    "cli_failed": "CLI failed",
    "ltvm_cleanup_abandoned": "LTVM cleanup abandoned",
    "ltvm_cleanup_failed": "LTVM cleanup failed",
    "ltvm_cleanup_stuck": "LTVM cleanup stuck",
    "ltvm_cleanup_succeeded": "LTVM cleanup succeeded",
    "ltvm_list_failed": "LTVM list failed",
    "ltvm_prefix_baseline": "LTVM prefix baseline",
    "ltvm_prefix_baseline_unprovable": "LTVM prefix baseline unprovable",
    "vm_stat_failed": "VM stat failed",
}
...
def _human(value):
    ...
    if text == UNKNOWN:
        return "Unknown"
    label = HUMAN_LABELS.get(_state(text))
    if label:
        return label
...
def _failure(run):
    """Return the recorded (code, summary) explaining why a run ended."""
    code = _get(run, "failure_code", "failure")
    summary = _get(
        run, "failure_summary", "failure_detail", "failure_message",
    )
    if not code and not summary:
        return None
    return _plain(code, ""), _plain(summary, "")


def _render_failure(run):
    """Render the stored failure reason, which no view used to show at all.

    A failed run's page said only "Run: Failed"; the controller records a real
    message ("no checkout available for this run: ..."), so show it first.
    """
    ...
    code_html = (
        f"<p class='failure-code'>Failure code: <code>{escape(code)}</code></p>"
        if code else ""
    )
    return (
        "<section class='run-failure' role='alert' aria-labelledby='run-failure-title'>"
        "<h3 id='run-failure-title'>Why this run ended</h3>"
        "<p class='failure-summary'>"
        + escape(summary or "No failure summary was recorded.")
        + "</p>" + code_html + "</section>"
    )


def _failure_line(run):
    """Render the same failure reason compactly, for the run card."""
    failure = _failure(run)
    if failure is None:
        return ""
    code, summary = failure
    code_html = f" <code>{escape(code)}</code>" if code else ""
    return (
        "<p class='run-failure-line'><strong>Why it ended:</strong> "
        + escape(summary or "No failure summary was recorded.")
        + code_html + "</p>"
    )
...
def render_investigate_control(
    ...
    return (
        ...
        f"{fields}" + _agent_choice_fields("investigate")
        + f"<button type='submit'{disabled}>Investigate</button></form>"
...
def _run_heading(kind):
    """Title one run card: "Engineering run", "Build repair run", "Agent run"."""
    text = str(kind or "").strip()
    if not text:
        return "Agent run"
    return text[0].upper() + text[1:] + " run"


def render_run_summary(run, *, base_url="/runs", kind=None):
    """Render a bounded overview of an active or historical run.

    ``kind`` names what the run is doing -- engineering, review comment, build
    repair -- because one card now lists every kind of run and a grid of
    identically headed "Agent run" cards said nothing about which was which.
    """
    ...
    return (
        ...
        f"<header><h3 id='{escape(title_id, quote=True)}'>"
        f"{escape(_run_heading(kind))}</h3>"
        ...
        + "</dl>" + _failure_line(run) + _countdowns(run)
...
def _follow_up_note(run):
    """State what a follow-up actually starts, which is never the same kind."""
    kind = _state(_get(run, "run_kind", "request_kind"))
    kind_text = ""
    # Saying so about an investigation would be false: that IS what it starts.
    if kind and kind not in {"", UNKNOWN, "investigation", "read_only_investigation"}:
        kind_text = f" It does not start another {_human(kind).casefold()} run."
    return (
        "<p class='control-note'>A follow-up is always a new read-only "
        "investigation pinned to the patch's current revision; it has no Gerrit "
        "or CI write capability." + escape(kind_text) + "</p>"
    )
...
def _render_controls(run, *, base_url, csrf_token, idempotency_token):
    ...
    if state in TERMINAL_STATES:
        # A follow-up always starts a read-only investigation, whatever kind of
        # run this was.  Labelling it "Start follow-up run" on a failed review
        # or build-repair run promised another run of that kind.
        forms.append(_simple_post_form(
            f"{path}/follow-up",
            "Start follow-up investigation without a message",
            "follow_up", run, csrf_token, idempotency_token,
        ))
        forms.append(_follow_up_note(run))
    return (
...
def _render_guidance(run, *, base_url, csrf_token, idempotency_token):
    ...
    if state in TERMINAL_STATES:
        help_text = (
            "This run is terminal. Guidance can only start a new read-only "
            "follow-up investigation, and this composer requires a message; "
            "Run controls below start the same investigation without one."
        )
        action = f"{path}/follow-up"
        modes = (
            "<button type='submit' name='delivery_mode' value='follow_up'>"
            "Start follow-up investigation with this message</button>"
        )
    elif state == "paused":
...
def render_run_detail(
    ...
    # The boundary text is a safety statement, so it follows the CAPABILITY
    # profile the agent was started with, never the session profile. A manual
    # investigation is minted with the "engineering" session profile and is
    # then granted `read_only`: Read/Glob/Grep, `--safe-mode --restricted`, and
    # service credentials scrubbed. Reading the session profile made that run
    # claim a host shell, real credentials, and its own Gerrit and CI writes.
    engineering = _state(
        _get(run, "capability_profile", "capabilities", default="read_only")
    ) in FULL_CAPABILITY_PROFILES
    heading = "Engineering run" if engineering else "Read-only investigation"
    boundary = (
        "<p class='safety-note'><strong>Engineering boundary:</strong> the run has a host "
        "shell, the installed LLM tools, and real service credentials, and makes its own "
        "Gerrit and CI writes. It is expected to stay inside its own checkout and the LTVM "
        "guests it owns.</p>"
        if engineering else
