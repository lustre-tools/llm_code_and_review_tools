"""Pure deterministic policy for Maloo retest decisions.
...
MAX_SUBTEST_NAME = 512
MAX_FAILING_SUBTESTS = 500
...
def _tolerated_text(value: Any, field: str, maximum: int) -> str:
    """Bound free text from Maloo instead of rejecting the whole snapshot.

    ``_require_text`` is right for values this module's own callers control.
    It is wrong for values Maloo hands us: the adapter deliberately tolerates
    an absent test group and passes through subtest names taken verbatim from
    test output, so rejecting them here made the two layers disagree and threw
    a raw ValueError out of ``tick_patch`` -- recording no observation, sending
    no notification, and leaving only a line in errors.jsonl.
    """
...
class MalooFailure:
    ...
    def __post_init__(self) -> None:
        ...
        # Maloo's ``review`` output carries no test group for some sessions and
        # the adapter passes the empty string through; this layer must accept
        # what that one emits.  The value only labels and groups a decision --
        # the retest itself is addressed by session id -- so an empty one is
        # safe.
        object.__setattr__(self, "test_group", _tolerated_text(self.test_group, "test_group", 256))
        ...
        # Subtest names are free text from test output: bound each one rather
        # than rejecting it.  Truncating a name keeps the failure identifiable,
        # so it does not by itself make the details incomplete.
        named = {
            _tolerated_text(item, "failing_subtest", MAX_SUBTEST_NAME)
            for item in self.failing_subtests
        }
        subtests = tuple(sorted(name for name in named if name))
        # A catastrophic patch really does fail more suites than this.  Keep a
        # bounded, deterministic prefix and mark the details incomplete: the
        # policy then routes the session to ``investigate`` and preserves it for
        # research instead of ever retesting on evidence we truncated.
        if len(subtests) > MAX_FAILING_SUBTESTS:
            subtests = subtests[:MAX_FAILING_SUBTESTS]
            object.__setattr__(self, "details_complete", False)
...
class RetestDecision:
    ...
    reason_code: str
    reason: str
    action: RetestActionPreview | None = None
...
def _action_key(snapshot: RevisionSnapshot, session_id: str) -> str:
    return (
        f"maloo-retest:{snapshot.change_number:d}:{snapshot.revision_sha}"
...
def _top_status(decisions: Sequence[RetestDecision]) -> tuple[str, str, str]:
    ...
    priorities = (
        ...
        ("investigate", "investigate_phase_2", "Unknown failures were preserved for research."),
...
def evaluate_retests(
    ...
    if not snapshot.is_current:
        return _global_result(
            snapshot,
            "stale_revision",
            f"Patchset {snapshot.patchset_number:d} revision "
            f"{snapshot.revision_sha} is not current; no retest is allowed.",
        )
    if snapshot.lifecycle in {"merged", "abandoned"}:
        return _global_result(
            snapshot,
            "terminal_change",
            f"Change {snapshot.change_number:d} is {snapshot.lifecycle}; "
            "terminal changes cannot be retested.",
        )
    ...
    if vetoes:
        visible = ", ".join(f"{vote.reviewer} ({vote.value:+d})" for vote in vetoes)
        return _global_result(
            snapshot,
            "non_maloo_review_veto",
            f"Current patchset {snapshot.patchset_number:d} has a non-Maloo "
            f"Code-Review veto from {visible}; the Maloo flow was not evaluated.",
        )
            ...
            decisions.append(
                RetestDecision(
                    **base,
                    ...
                    reason=f"Maloo retest {pending.remote_request_id} is already pending for session {session_id}.",
                )
            )
            continue
            ...
            decisions.append(
                RetestDecision(
                    **base,
                    ...
                    reason=(
                        "Session {} is not safe to retest because enforced suite(s) {} "
                        "lack complete accepted Jira-bug evidence; preserve the failure for research.".format(session_id, ", ".join(affected))
                    ),
                )
            )
            continue
        ...
        decisions.append(
            RetestDecision(
                **base,
                ...
                reason=(
                    f"Every enforced failed suite in session {session_id} has accepted Jira evidence; "
