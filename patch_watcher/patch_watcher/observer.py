...
class BackgroundObserver:
    ...
    def tick(self) -> bool:
        ...
        try:
            try:
                patches = list(self.patches_provider())
            except Exception as exc:
                # Outside the per-patch guard, this escaped `tick` entirely and
                # killed the polling thread for good: patches silently stopped
                # being refreshed while the web app kept serving stale rows.
    ...
    def _run(self) -> None:
        while not self._stop.wait(self.interval_seconds):
            try:
                ...
                # Backstop: even a failure in tick's own bookkeeping (or in the
                # error handler) must not end the loop. A dead poller is
                # invisible -- the dashboard keeps rendering, just never
                # updating.
                if isinstance(exc, (KeyboardInterrupt, SystemExit)):
                    raise
                # The error handler itself may be the thing that is broken.
