...
# How far a reported activity timestamp may fall behind the stored anchor and
# still be treated as ordinary out-of-order ingestion rather than a clock step.
ACTIVITY_REGRESSION_TOLERANCE = timedelta(minutes=5)
...
# Must be >= claude_runner.MAX_EVENT_BYTES. The runner truncates its own
# payloads above 256 KiB; this was 256_000, leaving a 6,144-byte window where
# the runner emitted an event the store then refused. Ingestion could not get
# past that cursor, so a COMPLETED run whose report sat further along was
# thrown away and the operator was told "event payload exceeds 256 KiB".
MAX_JSON_TEXT_BYTES = 256 * 1024


def _json_text(name: str, value: Any, *, maximum_bytes: int = MAX_JSON_TEXT_BYTES) -> str:
    try:
    ...
    if size > maximum_bytes:
        # Report real bytes; the old message called 256_000 bytes "256 KiB".
        raise ValueError(f"{name} is {size} bytes, over the {maximum_bytes}-byte limit")
    return encoded
...
class SessionStateStore:
    ...
    _MIGRATIONS: ClassVar[dict] = {
        ...
        # Version 3 created pw_worker_admission for the retired worker
        # admission subsystem. The step stays as a no-op so later versions
        # keep their numbering; databases that already ran it keep the
        # unused table instead of a destructive drop.
        3: (),
        # Marker lookups were scanning every runner_event row in the session:
        # measured at 161 ms per scan for a 20k-event run, paid several times
        # per tick against a 1.0s poll interval, so tick cost grew with history
        # until it stretched past the interval.
        5: (
            """
            CREATE INDEX IF NOT EXISTS pw_session_event_type_idx
            ON pw_session_event(session_id, event_type, event_id)
            """,
        ),
        # The model and reasoning effort a run was launched with. They belong
        # to the run, not the process: the operator picks them per run, and
        # the run detail page has to report what it actually used rather than
        # whatever the controller's default happens to be today.
        6: (
            "ALTER TABLE pw_managed_session ADD COLUMN model TEXT",
            "ALTER TABLE pw_managed_session ADD COLUMN effort TEXT",
        ),
        4: (
            ...
            """,
            """
            CREATE INDEX pw_session_event_type_idx
            ON pw_session_event(session_id, event_type, event_id)
            """,
            """
        ...
        # Both of these are FIFO queues that were ordered by a wall-clock
        # column with a random-UUID tiebreak, so a backward host clock step --
        # which WSL2 NTP drift does on this host -- could hand the consumer a
        # newer operator answer before an older pending one, or let a cancel
        # requested after a kill be applied first. pw_session_message hit the
        # same bug and fixed it by ordering on its INTEGER AUTOINCREMENT id;
        # these two get the same surrogate, and the queue reads order by it.
        # 'follow_up' also leaves the action CHECK: request_follow_up was its
        # only producer in any revision of this tree and never had a caller,
        # so no stored row can carry it and the backfill needs no filter.
        7: (
            "ALTER TABLE pw_outbound_guidance RENAME TO pw_outbound_guidance_v6",
            """
            CREATE TABLE pw_outbound_guidance (
            ...
            """,
            # ROW_NUMBER over the ordering the queue used until now, so an
            # existing row keeps the position it already had.
            """
            INSERT INTO pw_outbound_guidance(
                sequence, guidance_id, session_id, body, status,
                idempotency_key, created_at, claimed_by, claimed_at,
                delivered_at, failed_at, failure_summary
            )
            SELECT
                ROW_NUMBER() OVER (ORDER BY created_at, guidance_id),
                guidance_id, session_id, body, status, idempotency_key,
                created_at, claimed_by, claimed_at, delivered_at, failed_at,
                failure_summary
            FROM pw_outbound_guidance_v6
            """,
            "DROP TABLE pw_outbound_guidance_v6",
            """
            CREATE INDEX pw_outbound_guidance_pending_idx_v7
            ON pw_outbound_guidance(session_id, status, sequence)
            """,
            "ALTER TABLE pw_session_control_intent RENAME TO pw_session_control_intent_v4",
            """
            CREATE TABLE pw_session_control_intent (
                sequence INTEGER PRIMARY KEY AUTOINCREMENT,
                ...
                session_id TEXT NOT NULL REFERENCES pw_managed_session(session_id)
                    ON DELETE CASCADE,
                action TEXT NOT NULL CHECK (
                    action IN ('pause', 'interrupt', 'cancel', 'kill')
                ),
                requested_by TEXT NOT NULL,
                requested_at REAL NOT NULL,
                status TEXT NOT NULL DEFAULT 'recorded' CHECK (
            ...
            """
            INSERT INTO pw_session_control_intent(
                sequence, request_id, session_id, action, requested_by,
                requested_at, status, detail_json, confirmed_by, confirmed_at,
                executed_at, failure_code, failure_summary,
                confirmation_token_hash, confirmation_expires_at,
                confirmation_used_at
            )
            SELECT
                ROW_NUMBER() OVER (ORDER BY requested_at, request_id),
                request_id, session_id, action, requested_by, requested_at,
                status, detail_json, confirmed_by, confirmed_at, executed_at,
                failure_code, failure_summary, confirmation_token_hash,
                confirmation_expires_at, confirmation_used_at
            FROM pw_session_control_intent_v4
            """,
            "DROP TABLE pw_session_control_intent_v4",
            """
    ...
    @staticmethod
    # NOTE: the `"col" in row.keys()` checks below must NOT be "simplified" to
    # `"col" in row`. These rows are sqlite3.Row, whose __contains__ tests
    # VALUES, not keys, so the short form silently returns False for every
    # column and the field reads back as None. Ruff's SIM118 offers exactly
    # that rewrite; it is wrong here and is suppressed for this file.
    def _session_from_row(row: sqlite3.Row) -> ManagedSession:
        ...
        return ManagedSession(
            ...
            # Rows written before migration 6 hold NULL here, and every
            # reader treats these as strings -- an un-coerced None reaches the
            # run page as the literal text "None".
            model=(row["model"] or "") if "model" in row.keys() else "",
            effort=(row["effort"] or "") if "effort" in row.keys() else "",
        )
    ...
    def set_state(
        ...
        *,
        ...
        with self._connection() as connection, connection:
            # sqlite3's legacy isolation only opens a transaction before DML,
            # never before a SELECT, so without this the terminal guard below
            # and the UPDATE are two separate transactions. A run reaped by
            # the controller between them came back to life: the terminal
            # result said "failed" while pw_managed_session said "running",
            # and nothing repaired it -- finish_session returns its existing
            # row early, so every later tick re-derived the same verdict and
            # got the same early return. The session never reached cleanup,
            # so its run directory, its LTVM guests and its checkout were
            # held until someone edited the database by hand.
            connection.execute("BEGIN IMMEDIATE")
            ...
            if cursor.rowcount != 1:
                # Belt and braces behind BEGIN IMMEDIATE: if the row moved
                # anyway, fail loudly rather than write a state derived from
                # a snapshot that no longer holds.
                raise InvalidSessionOperation(
                    f"session {session_id} changed state during the transition "
                    f"from {old_state} to {state}"
                )
        return self.get_session(session_id)
    ...
    def record_activity(
        ...
        with self._connection() as connection, connection:
            ...
            # MAX() alone pins the anchor to the highest timestamp ever seen,
            # so a single timestamp from the future freezes the inactivity
            # deadline until real time catches up to it. The host clock
            # stepping backwards does exactly that -- WSL2 NTP drift does it
            # routinely, see the build gotchas in CLAUDE.md -- and a wedged
            # session then holds its checkout until the 48h absolute cap
            # instead of the 30m inactivity limit. Regressions inside the
            # tolerance are ordinary out-of-order event ingestion and still
            # clamp; a larger one is a clock step, and re-anchoring is the
            # only way back. updated_at stays monotonic either way: it orders
            # listings and gains nothing from moving backwards.
            stored_activity = float(row["last_qualifying_activity_at"])
            if (
                stored_activity - activity_epoch
                > ACTIVITY_REGRESSION_TOLERANCE.total_seconds()
            ):
                connection.execute(
                    """
                    UPDATE pw_managed_session
    ...
    def reanchor_activity(self, session_id: str, *, at: datetime) -> ManagedSession:
        """Move every inactivity anchor to `at`, in either direction.

        `record_activity` refuses to move an anchor backwards by less than
        ACTIVITY_REGRESSION_TOLERANCE, which is right for out-of-order event
        ingestion and wrong when the host clock has demonstrably stepped:
        every stored stamp is then from a timeline that no longer exists.
        `active_interval_started_at` is the sharper half of that -- set_state
        writes it from the wall clock and nothing else ever moves it, so it
        outlives a backward step in the future and wins the max() in
        evaluate_policy, holding the inactivity deadline out of reach for the
        size of the step.

        Only a caller that has PROVED a step (by comparing wall time against a
        monotonic clock) may use this: it is a deliberate loss of history, and
        applying it on ordinary jitter would hide real inactivity.
        """

        anchor_epoch = _as_epoch(at)
        with self._connection() as connection, connection:
            connection.execute("BEGIN IMMEDIATE")
            row = self._require_session(connection, session_id)
            if row["state"] in TERMINAL_STATES:
            ...
            connection.execute(
                ...
                SET last_qualifying_activity_at = ?,
                    active_interval_started_at =
                        CASE WHEN active_interval_started_at IS NULL
                             THEN NULL ELSE ? END,
                    updated_at = MAX(updated_at, ?)
                WHERE session_id = ?
                """,
                (anchor_epoch, anchor_epoch, anchor_epoch, session_id),
            )
        return self.get_session(session_id)
        ...
        with self._connection() as connection, connection:
            ...
            connection.execute(
                ...
                WHERE message_id IN (
                    ...
                    WHERE session_id = ?
                    -- message_id, not created_at: created_at is the runner's
                    -- wall clock, so after a backward clock step every new
                    -- message sorts OLDEST and the bounded tail drops the one
                    -- just inserted -- far enough back and the INSERT's own
                    -- row is pruned by its own DELETE, tripping the assert
                    -- below and aborting ingestion once per message for as
                    -- long as the skew lasts. message_id is insertion order
                    -- by construction, which is what "the newest 50" means.
    ...
    def recent_messages(
        ...
        with self._connection() as connection:
            ...
            rows = connection.execute(
                ...
                WHERE session_id = ?
                -- Same reason as the retention query above.
                ORDER BY message_id DESC
                LIMIT ?
                """,
    ...
    def list_events(
        ...
        *,
        ...
        """Return this session's events, oldest first.

        ``event_types`` narrows the read to specific types. Prefer it in any
        hot path looking for a marker event: a run accumulates one
        ``runner_event`` per Claude stream line (thousands), while the markers
        the controller looks for number a handful, and an unfiltered scan
        ...
        query = (
            "SELECT * FROM pw_session_event WHERE session_id = ? AND event_id > ?"
        )
        parameters: list[Any] = [session_id, after_event_id]
        if event_types is not None:
            wanted = [str(item) for item in event_types]
            if not wanted:
                return []
            query += f" AND event_type IN ({', '.join('?' for _ in wanted)})"
        ...
        with self._connection() as connection, connection:
            ...
            if existing is not None:
                ...
                if session["state"] != existing["state"]:
                    # The two rows disagree, so an older build raced them
                    # apart. Returning early without repairing the session row
                    # is what made that permanent: the run stayed "active"
                    # forever and never released its checkout. Heal it here so
                    # an already-wedged database recovers on the next tick.
                    connection.execute(
                        """
                        UPDATE pw_managed_session
                        ...
                        WHERE session_id = ?
                        """,
                        (existing["state"], finished_epoch, session_id),
                    )
                return self._terminal_from_row(existing)
    ...
    def mark_resource_cleanup(
        ...
        *,
        ...
        with self._connection() as connection, connection:
            ...
            if row["state"] == target:
                # `cleaned` is final, so a repeat is genuinely a no-op. A repeat
                # `cleanup_failed` is not: cleanup retries, and each attempt has
                # its own reason. Early-returning here silently discarded every
                # reason after the first, so the row kept the oldest failure
                # while the real one -- including the give-up -- lived only in
                # the event log.
                if succeeded or failure_summary is None:
                    return self._resource_from_row(row)
                if row["cleanup_failure"] == failure_summary:
                    return self._resource_from_row(row)
