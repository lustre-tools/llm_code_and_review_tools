...
# A Maloo answer larger than this is a fault, not data.  The runner reads one
# byte past the bound (the way jenkins_adapter._default_transport does) so an
# oversized response is rejected without ever being fully materialized: a
# 108 MiB body previously parsed into ~1.5M dataclasses and took peak RSS from
# 14 MiB to 767 MiB, and ``timeout`` never fired because the read completed.
MAX_OUTPUT_BYTES = 8 * 1024 * 1024
MAX_STDERR_BYTES = 64 * 1024
COMMAND_TIMEOUT = 45.0
# Identifiers are interpolated into argv and, remotely, into URL paths.  The
# character set is deliberately narrower than "not obviously hostile": a NUL
# made subprocess raise a bare ValueError, and ``..`` in a value Maloo may use
# to build a path is refused even though the argv itself is inert.
...
def redact_text(value: Any) -> str:
    ...
    return _SECRET_PATTERNS[2].sub("[REDACTED AUTHORIZATION]", text)
...
@dataclass(frozen=True)
class CommandResult:
    """What an injected ``Runner`` returns.

    The adapter never names this type internally -- it duck-types on the
    attributes -- but it is the documented shape of the runner contract and is
    what callers and tests construct. Do not delete it as "unreferenced".
    """
...
def _read_bounded(
    stream: Any, limit: int, deadline: float, argv: Sequence[str]
) -> tuple[bytes, bool]:
    """Read at most ``limit`` bytes from ``stream``, honouring ``deadline``.

    ``subprocess.run`` reads a child to EOF, so a runaway answer is fully
    resident before this module can reject it.  Reading with an explicit
    bound keeps the cost of a bad response constant, and the selector keeps
    the timeout that ``subprocess.run`` would otherwise have provided.
    """
    ...
    try:
        while total < limit:
            ...
            if remaining <= 0 or not selector.select(remaining):
                raise subprocess.TimeoutExpired(list(argv), COMMAND_TIMEOUT)
            chunk = stream.read1(min(65536, limit - total))
            if not chunk:
                break
            chunks.append(chunk)
            total += len(chunk)
...
def _default_runner(argv: Sequence[str]) -> CommandResult:
    """Run the Maloo CLI with bounded, never-failing output decoding.

    ``text=True`` without ``errors=`` made a single non-UTF-8 byte anywhere in
    Maloo's answer raise UnicodeDecodeError.  That is a ValueError, so it flew
    past ``_invoke``'s typed handlers -- and after ``maloo retest`` had already
    run, an at-most-once mutation whose outcome was unknown was recorded as a
    definitive local failure that reconciliation then skipped.  Bytes are read
    here and decoded with replacement, so decoding cannot fail at all.

    stderr is written to a temporary file rather than a pipe so the child can
    never block on a full stderr buffer while stdout is being read under the
    bound above.
    """
    ...
    with tempfile.TemporaryFile() as error_file:
        with subprocess.Popen(
            ...
            try:
                stdout, overflowed = _read_bounded(
                    process.stdout, MAX_OUTPUT_BYTES + 1, deadline, argv
                )
                remaining = max(0.0, deadline - time.monotonic())
                if overflowed:
                    # Reaping a child we killed for overflow is not the
                    # caller's timeout, so it always gets a moment.
                    process.kill()
                ...
                raise
        error_file.seek(0)
        stderr = error_file.read(MAX_STDERR_BYTES)
...
def _identifier(value: str, label: str) -> str:
    ...
    if len(result) > 200 or not _IDENTIFIER_RE.fullmatch(result):
        raise MalooAdapterError(
...
def _normalize_retest_fields(data: Mapping[str, Any]) -> tuple[bool | None, str, str]:
...
class MalooAdapter:
    ...
    def _invoke(self, operation: str, args: Sequence[str], *, mutation: bool = False) -> Mapping[str, Any]:
        ...
        try:
            ...
            # A runner that decodes the child's output itself can still fail on
            # a non-UTF-8 byte.  UnicodeDecodeError is a ValueError, so without
            # this it escaped the typed machinery entirely.  The command has
            # already run by the time decoding fails, so a mutation's remote
            # outcome is genuinely unknown and must be reconciled -- it is not
            # a definitive local failure.
            if mutation:
                raise MalooAdapterError(
                    MalooErrorCode.AMBIGUOUS_MUTATION, operation,
                    f"Maloo {operation} output could not be decoded; the remote outcome is unknown and must be reconciled",
                    ambiguous=True,
                ) from exc
            raise MalooAdapterError(
                MalooErrorCode.INVALID_RESPONSE, operation,
                "Maloo returned output that could not be decoded",
            ) from exc
        ...
        if isinstance(stdout, (str, bytes, bytearray)) and len(stdout) > MAX_OUTPUT_BYTES:
            # Reject before parsing: the blow-up is json.loads plus the
            # dataclasses it feeds, not the read itself.
            raise MalooAdapterError(
                MalooErrorCode.AMBIGUOUS_MUTATION if mutation else MalooErrorCode.INVALID_RESPONSE,
                operation,
                f"Maloo {operation} returned more than {MAX_OUTPUT_BYTES} bytes"
                + (
        ...
        if isinstance(meta, Mapping) and (
            meta.get("tool") not in (None, "maloo")
            or meta.get("command") not in (None, operation)
        ):
                raise MalooAdapterError(
                    MalooErrorCode.AMBIGUOUS_MUTATION if mutation else MalooErrorCode.INVALID_RESPONSE,
                    operation,
    ...
    def get_bug_links(self, buggable_id: str, *, related: bool = True) -> MalooBugLinks:
        ...
        links = normalize_bug_links(self._invoke("bugs", args))
        # Verify the answer is about the suite we asked about. The WRITE paths
        # already check that Maloo echoed the association they requested; this
        # read did not, so links belonging to a different test set would be
        # accepted as this one's. A caller then reports an existing bug link
        # and skips writing one -- leaving the real suite unbugged -- or
        # issues a retest on the strength of an association that does not
        # exist for it.
        if links.buggable_id != target:
            raise MalooAdapterError(
                MalooErrorCode.INVALID_RESPONSE, "bugs",
                "Maloo answered about a different buggable: asked "
                f"{target}, got {links.buggable_id}",
            )
        return links
    ...
    def reconcile_remote_retest(
        ...
        *,
        ...
        jira_ticket: str = "",
        queue_evidence: MalooQueueEvidence | None = None,
        review_evidence: MalooReviewSessions | None = None,
    ) -> MalooRetestReconciliation:
