"""Compact, side-effect-free views for review-comment handling."""
...
def render_review_start_control(
    ...
    action: str = "/review-runs/prepare",
):
    eligible = bool(
        patch.get("revision_sha") and patch.get("revision_ref")
...
def render_review_start_confirmation(
    ...
    return (
        ...
        "<p>This approval starts a writable run with a host shell, the installed LLM tools, "
        "and real service credentials. When its work is complete and validated, the run posts "
        "the replies and uploads the new patchset itself with the gerrit CLI. The controller "
        "does not upload on its behalf. There is no later upload confirmation. The run is "
        "expected to stay inside its own checkout and its own VMs.</p>"
        f"<form method='post' action='{escape(action, quote=True)}'>"
        f"<input type='hidden' name='csrf_token' value='{escape(csrf_token, quote=True)}'>"
...
def render_review_result(request, report):
    ...
    return (
        ...
        "<p>A complete run posts its own replies with the gerrit CLI; "
        "the controller posts nothing.</p><ul>"
