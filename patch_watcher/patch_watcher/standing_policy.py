...
# The automatic path uses a decision's coalescing key as the run's request
# identity, so this prefix is the one durable mark that separates a run this
# module caused from one an operator started by hand.  A controller that has
# to bound automation needs to tell those apart, and inventing a second flag
# for it would be a claim the caller could get wrong.
STANDING_TRIGGER_PREFIX = "standing:"
...
def trigger_coalescing_key(
    ...
    *,
    ...
    return STANDING_TRIGGER_PREFIX + hashlib.sha256(encoded).hexdigest()


def is_standing_trigger_key(value: Any) -> bool:
    """True when a request identity was minted by an automatic standing trigger."""

    return isinstance(value, str) and value.startswith(STANDING_TRIGGER_PREFIX)
    ...
    def _locked(self, *, exclusive: bool):
        ...
        class _Lock:
            def __enter__(self):
                ...
                # The chmod and the flock can both fail -- a lock file left
                # behind by another user, a filesystem that will not flock --
                # and an exception out of __enter__ means __exit__, which holds
                # the close, never runs.  Every dashboard render and every
                # policy POST takes this lock, so that leaked exactly one
                # descriptor per failed acquire: 200 induced failures, 200
                # descriptors, until the process hit its file limit and the
                # dashboard stopped answering.  Own the descriptor locally and
                # publish it only once the lock is actually held.
