...
def parse_watch_request(text):
    """Split what the operator typed into a JIRA key and change numbers.

    One box, because "add a patch", "add a series" and "add a ticket" are the
    same intent with different contents, and a form that made you choose first
    would be asking you to classify your own paste.  URLs, bare numbers,
    commas, whitespace and newlines all work; a Gerrit URL keeps only its
    change number, so a link copied with a patchset or a file path on the end
    still names one change.
    """
    ...
    for token in tokens:
        if not token:
            continue
        key = token.upper()
        if TICKET_KEY_RE.fullmatch(key):
            if ticket and ticket != key:
                unreadable.append(token)
                continue
            ticket = key
            continue
        found = re.search(r"/\+/(\d+)", token) or re.fullmatch(r"(\d+)", token)
        if not found:
            found = re.search(r"/c/(\d+)(?:\D|$)", token)
        if found:
            ...
            continue
        unreadable.append(token)
    return ticket, changes, unreadable


def add_watch_request(text, kind="", label=""):
    """Add one patch, a declared group of them, or a ticket and its changes.

    Returns (message, patches) -- the message is what the page should say.
    """
    ticket, changes, unreadable = parse_watch_request(text)
    if unreadable:
        return (
            "Could not read: " + ", ".join(unreadable[:5])
            + ". Give Gerrit URLs or change numbers, or one JIRA key."
        ), []
    if not ticket and not changes:
        return "Nothing to add. Give a Gerrit URL, a change number, or a JIRA key.", []
    if ticket and PATCH_GROUP_STORE is None:
        return "Group storage is unavailable, so a ticket cannot be watched.", []

    # A ticket brings its own changes with it, on top of anything typed.
    discovered = []
    if ticket:
        try:
            discovered = [
                str(item["change_number"])
                for item in GerritStatusClient.configured().changes_for_ticket(ticket)
            ]
        except (GerritConfigError, GerritRequestError) as exc:
            return f"Could not ask Gerrit what {ticket} has: {exc}", []
    ...
    for number in changes:
        patch, error = add_patch(f"https://review.whamcloud.com/c/{number}")
        if error and not patch:
            return f"Could not add change {number}: {error}", added
        if patch is not None:
            added.append(patch)

    if ticket:
        group = PatchGroup(ticket, members=changes, kind="ticket", label=label)
    elif len(changes) > 1:
        wanted = (kind or "series").strip().casefold()
        if wanted not in {"series", "flock"}:
            return f"Unknown group kind {kind!r}.", added
        group = PatchGroup(changes[0], members=changes, kind=wanted, label=label)
    else:
        return f"Watching change {changes[0]}.", added

    try:
        current = PATCH_GROUP_STORE.get(group.group_id)
        saved = PATCH_GROUP_STORE.save(
            group, expected_version=current.version if current else 0
        )
    except PatchGroupError as exc:
        return f"Added the changes, but not the group: {exc}", added
    if saved.kind == "ticket":
        found = f", and found {len(discovered)} change(s) on it" if discovered else ""
        return f"Watching {saved.ticket}{found}.", added
    return (
        f"Watching {len(saved.members)} changes as one {saved.kind}: "
        + ", ".join(saved.members) + "."
    ), added
...
def runs_html(limit=25):
    ...
    spend = _usage_line(_usage_totals(SESSION_STORE.list_usage()), prefix="Spend across every run: ")
    spend_html = f"<p class='detail runs-spend'>{spend}</p>" if spend else ""
    return (
...
def run_detail_html(session):
    ...
    return render_run_detail(
        ...
        csrf_token=CSRF_TOKEN,
        idempotency_token=secrets.token_urlsafe(18),
    ) + _run_usage_html(session) + review_html + build_html + _engineering_detail_html(session)


def _run_usage_html(session):
    """What this one run cost, as the CLI reported it."""
    if SESSION_STORE is None:
        return ""
    usage = SESSION_STORE.get_usage(session.session_id)
    if usage is None:
        return ""
    divisor = _subscription_divisor()
    rows = [
        ("List cost", _format_cost(usage.cost_usd)),
        (f"On subscription (÷{divisor:g})", _format_cost(usage.cost_usd / divisor)),
        ("Model", usage.model),
        ("Turns", str(usage.turns)),
        ("Wall time", f"{usage.duration_ms / 1000:.0f}s"),
        ("Input tokens", _format_tokens(usage.input_tokens)),
        ("Output tokens", _format_tokens(usage.output_tokens)),
        ("Cache read", _format_tokens(usage.cache_read_tokens)),
        ("Cache write", _format_tokens(usage.cache_creation_tokens)),
        ("Total tokens", _format_tokens(usage.total_tokens)),
    ]
    return (
        "<section class='run-usage' aria-labelledby='run-usage-title'>"
        "<h3 id='run-usage-title'>What this run cost</h3><dl>"
        + "".join(
            f"<div><dt>{escape(label)}</dt><dd>{escape(str(value))}</dd></div>"
            for label, value in rows
        )
        + "</dl><p class='detail'>The list cost and token counts are what the "
        "Claude CLI reported for this run. The subscription figure is that cost "
        "divided by the configured divisor -- an estimate, not a measurement.</p>"
        "</section>"
    )
...
# reports.  A guess, and labelled as one wherever it is shown: the list figure
# is what was measured, this is only what it is worth dividing by.
SUBSCRIPTION_DIVISOR = 37.0


def _subscription_divisor():
    """The divisor, overridable in the private config."""
    try:
        value = float(GerritConfig.load().subscription_divisor)
    except (GerritConfigError, AttributeError, TypeError, ValueError):
        return SUBSCRIPTION_DIVISOR
    return value if value > 0 else SUBSCRIPTION_DIVISOR
...
def _usage_totals(records):
    """Sum a set of run costs into one reportable figure."""
    records = list(records)
    cost = sum(item.cost_usd for item in records)
    divisor = _subscription_divisor()
    return {
        "runs": len(records),
        "cost_usd": cost,
        "effective_usd": cost / divisor if divisor else cost,
        "divisor": divisor,
...
def _usage_line(totals, *, prefix=""):
    """One line an operator can read: list price, what it is worth on a
    subscription, and the tokens both come from."""
    if not totals["runs"]:
        return ""
    divisor = totals["divisor"]
    return (
        f"<span class='usage-line'>{escape(prefix)}"
        f"<strong>{escape(_format_cost(totals['effective_usd']))}</strong> "
...
def _patch_usage_totals(patch):
    ...
    if change_number is None:
        return _usage_totals([])
    return _usage_totals(SESSION_STORE.list_usage(str(change_number)))
...
def _patch_run_html(patch):
    ...
    This used to live inside the collapsed "Actions for this patch"
    disclosure, so the one thing an operator most wants at a glance -- is
    something running on this patch right now, and can I see what it is doing
    -- took a click to discover, and the only live view was a card far down
    the page.  Running or not, it is stated here, with the controls that act
    on it.
    """
    if SESSION_STORE is None:
        return ""
    ...
    if active is None:
        ...
        spend_html = f"<div class='detail patch-spend'>{spend}</div>" if spend else ""
        return (
            "<div class='patch-run watching'>"
            + _chip("Watching", "info")
            + f" <span class='detail'>{heartbeat}</span>"
            f"<div class='detail watch-why'>{explanation}</div>"
            f"{last_html}{spend_html}</div>"
        )

    href = "/runs/" + escape(active.run_id, quote=True)
    version = _run_version(active)
    token = secrets.token_urlsafe(18)
    state = active.state.replace("_", " ")
    tone = "warn" if active.state == "waiting_human" else "good"
    elapsed = ""
    started = active.started_at
    if started is not None:
        seconds = max(0, int((datetime.now(UTC) - started).total_seconds()))
        elapsed = f" · running {seconds // 60}m {seconds % 60}s" if seconds >= 60 else f" · running {seconds}s"
    latest = ""
    messages = SESSION_STORE.recent_messages(active.session_id, limit=1)
    if messages:
        body = " ".join(str(messages[-1].body or "").split())
        latest = (
            f"<div class='detail run-latest'>{escape(body[:180])}"
            + ("…" if len(body) > 180 else "") + "</div>"
        )
    waiting = (
        "<div class='detail run-failure-line'><strong>Waiting for your decision.</strong></div>"
        if active.state == "waiting_human" else ""
    )
    spend = ""
    usage = SESSION_STORE.get_usage(active.session_id)
    if usage is not None:
        spend = (
            "<div class='detail patch-spend'>"
            + _usage_line(_usage_totals([usage]), prefix="So far: ") + "</div>"
        )

    def control(path, label, intent):
        return (
            f"<form class='run-control' method='post' action='{href}/{path}'>"
            f"<input type='hidden' name='intent' value='{intent}'>"
            f"<input type='hidden' name='expected_version' value='{version}'>"
            f"<input type='hidden' name='csrf_token' value='{escape(CSRF_TOKEN, quote=True)}'>"
            f"<input type='hidden' name='idempotency_token' value='{escape(token, quote=True)}'>"
            f"<button type='submit'>{escape(label)}</button></form>"
        )

    return (
        "<div class='patch-run live'>"
        f"{_chip('Run: ' + state.capitalize(), tone)} "
        f"<a href='{href}'><strong>{escape(_run_kind(active.run_id))} run</strong></a>"
        f"<span class='detail'>{escape(elapsed)}</span>"
        f"{waiting}{latest}{spend}"
        "<div class='run-controls-inline'>"
        f"<a class='button-link' href='{href}'>Open run</a>"
        + control("pause", "Pause", "pause")
        + control("interrupt", "Interrupt", "interrupt")
        + f"<a class='danger-link' href='{href}/confirm?intent=cancel&amp;version={version}'>Stop…</a>"
        "</div></div>"
    )
...
def _patch_now_html(patch):
    ...
    # The run itself is on the row, above this disclosure; this line is about
    # what will happen without being asked.
    run_html = ""
    if active is not None:
        href = "/runs/" + escape(active.run_id, quote=True)
        run_html = (
            f"<a href='{href}'>{escape(active.run_id)}</a> is "
            f"{escape(active.state.replace('_', ' '))}."
        )
    try:
        policy = _standing_policy(patch)
        if policy.rank == 0:
...
def _patch_row(patch, jira_base=JIRA_BASE_URL):
    ...
    # Poll this one patch on demand.  Everything on the row is as of the
    # last check, and the row says how long ago that was; without this the
    # only ways to ask again were Refresh all and waiting out the interval.
    check_now_html = (
        "<form class='check-now' method='post' action='/check-now'>"
        f"<input type='hidden' name='csrf_token' value='{escape(CSRF_TOKEN, quote=True)}'>"
        f"<input type='hidden' name='change_number' value='{escape(str(patch.get('change_number') or ''), quote=True)}'>"
        "<button class='secondary' type='submit' title='Read Gerrit for this "
        "patch now and apply its level, exactly as the next check would'>"
        "Check now</button></form>"
    )
    action_policy_html = (
        "<div class='patch-controls'>" + check_now_html
...
def page(message="", jira_base=JIRA_BASE_URL):
    ...
    # The number an operator wants at the top of the page is what this has
    # cost them, which on a subscription is not the list price.
    fleet = _usage_totals(SESSION_STORE.list_usage()) if SESSION_STORE is not None else _usage_totals([])
    if fleet["runs"]:
        spend_total_html = (
            "<div class='detail total-spend'>Spent so far: <strong>"
            + escape(_format_cost(fleet["effective_usd"]))
            + f"</strong> on subscription (÷{fleet['divisor']:g}) · "
            + escape(_format_cost(fleet["cost_usd"])) + " list · "
            + escape(_format_tokens(fleet["tokens"])) + " tokens across "
            + f"{fleet['runs']} run{'s' if fleet['runs'] != 1 else ''}</div>"
        )
    else:
        spend_total_html = ""
    with PATCHES_LOCK:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _dispatch_get(self):
        ...
        if len(parts) >= 2 and parts[0] == "runs":
            ...
            if len(parts) == 3 and parts[2] == "messages":
                # The chat panel polls this and swaps in the transcript, so a
                # live run reads as a conversation without reloading the page
                # and discarding whatever the operator is halfway through
                # typing.
                self.respond(render_chat_messages(_run_messages(session)))
                return
    ...
    def _dispatch_post(self):
        ...
        if path == "/resources/refresh":
            ...
            # One patch, doing exactly what the periodic check does to it:
            # read Gerrit, then apply its level.  Anything less would make
            # the button mean something different from the sentence on the
            # row it sits under, which says what the next check will do.
            #
            # Identified by change number alone, deliberately.  The other
            # per-patch controls pin change, patchset and revision because
            # they act on an exact revision; this one goes and finds out what
            # the revision is, so refusing a stale one would refuse exactly
            # when it is most useful.
            try:
                change = int(data.get("change_number", ["0"])[0])
            except ValueError:
                self.send_error(400, "Invalid patch identity")
                return
            target = next(
                (
                    item for item in _patch_snapshot()
                    if int(item.get("change_number") or 0) == change
                ),
                None,
            )
            if target is None:
                self.send_error(409, "That patch is no longer watched")
                return
            error = refresh_watched_patch(target)
            if error is None:
                _observe_patch_automation(target)
            else:
                self.respond(page(f"Could not check {change}: {error}"))
                return
...
def main(argv=None):
    ...
    # It binds to loopback and has no authentication of its own, so the way to
    # reach it from elsewhere is a tunnel, not a wider bind: over SSH the
    # browser still talks to localhost, which is what the Host check wants.
    print(
        f"  From another machine:  ssh -N -L {args.port}:127.0.0.1:{args.port} "
        f"{os.environ.get('USER', 'you')}@{platform.node()}"
    )
    print(f"  then open http://127.0.0.1:{args.port} there.")
