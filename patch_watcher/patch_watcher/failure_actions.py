...
Revalidate = Callable[
    [str], FailurePatchRevision | Mapping[str, Any] | Any
]
    ...
    def _execute_link(
        ...
        try:
            ...
            if not result.linked:
                # Maloo answered and declined: nothing further will arrive, so
                # recording this as waiting_external would park the run for
                # good.  This mirrors the retest path's `requested` check.
                return self._definitive_error(
                    run, action, "link_rejected", "Maloo rejected the bug link"
                )
            self.store.finish_action(
    ...
    def _execute_retest(
        ...
        try:
            ...
            if not result.requested:
                # Maloo answered and declined: nothing further will arrive, so
                # recording this as waiting_external would park the run for
                # good.  Same rule as the link write above.
                return self._definitive_error(
                    run, action, "retest_rejected", "Maloo rejected the retest"
                )
            self.store.finish_action(
                action.action_id, "waiting_external", result=result.to_dict()
            )
