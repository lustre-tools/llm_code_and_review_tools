...
def render_build_start_confirmation(
    ...
    *,
    ...
    return (
        ...
        + "</code>.</p>"
        "<p>This single approval starts a writable run with open-ended commands, a host "
          "shell, the installed LLM tools, and real service credentials. It creates and cleans up "
          "its own LTVM guests, and is expected to stay inside its own checkout and those guests.</p>"
        "<p>If the failure is caused by this patch and the fix builds and tests successfully, the "
          "run uploads the new patchset itself with the gerrit CLI. The controller does not upload "
          "on its behalf. <strong>There is no later upload confirmation.</strong> Any uncertainty "
          "escalates to a human instead of an upload.</p>"
        f"<form method='post' action='{escape(action, quote=True)}'>"
        f"<input type='hidden' name='csrf_token' value='{escape(csrf_token, quote=True)}'>"
        f"<input type='hidden' name='change_number' value='{escape(_plain(_get(patch, 'change_number')), quote=True)}'>"
        f"<input type='hidden' name='patchset' value='{escape(_plain(_get(patch, 'patchset')), quote=True)}'>"
...
def render_build_result(request, report):
    """Render diagnosis, validation, and escalation for a build run."""
