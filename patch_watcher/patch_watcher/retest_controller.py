"""Deterministic orchestration for Maloo retests.
...
try:  # Support both package and direct-module test imports.
    ...
    from patch_watcher.automation_state import (  # type: ignore
        AutomationConflict,
        AutomationRun,
        AutomationStateStore,
        GlobalAutomationDisabled,
    )
    from patch_watcher.maloo_adapter import (  # type: ignore
        MalooAdapter,
        MalooAdapterError,
        MalooEnforcedSessionFailure,
        MalooRetestReconciliation,
    )
    from patch_watcher.retest_policy import (  # type: ignore
        JiraBugLink,
        MalooFailure,
        PendingRetest,
        RetestBudget,
...
Revalidate = Callable[[str], PatchRevision | Mapping[str, Any]]
...
class RetestController:
        ...
        for suite in group.failures.failed_suites:
            ...
            if evidence is not None:
                ...
                for link in evidence.bugs.links:
                    try:
                        accepted.append(JiraBugLink(
                            issue_key=link.ticket,
                            accepted_for_retest=link.accepted,
                            evidence=f"maloo:{link.state}",
                        ))
                    except ValueError:
                        # Maloo accepts any non-empty ticket string, JiraBugLink
                        # requires a real Jira key, and that validation is what
                        # makes a retest justification trustworthy -- so the
                        # link is dropped rather than the check weakened. It
                        # used to raise straight out of tick_patch, losing the
                        # whole patch's observation cycle to one odd ticket.
                        dropped_links = True
            ...
            output.append(
                MalooFailure(
                    ...
                    # Dropping a link makes the bug evidence incomplete, and
                    # incomplete evidence must not read as "no bug is linked":
                    # that is the state an automatic retest is allowed from.
                    bug_links_complete=evidence is not None and not dropped_links,
        ...
        for run in self.store.list_runs(patch_id=patch_id, include_terminal=False):
            ...
            for action in actions:
                if action.status == "executing" and action.claimed_by == self.worker_id:
                    ...
                    self._poll_waiting(run, action)
            # Re-read: the reconcile/poll calls above may have advanced it.
            refreshed = self.store.get_run(run.run_id)
            if refreshed.status in {"planned", "executing", "waiting_external"}:
                self._execute_next_planned(refreshed)
                self._settle_run(refreshed.run_id)
        ...
        try:
            ...
            self._notify(
                ControllerNotification(
                    "retest_requested",
                    run.patch_id,
                    "Maloo retest requested for {}".format(request["session_id"]),
                    run.run_id,
                    action.action_id,
                    result.to_dict(),
                )
    ...
    def _record_reconciled(
        ...
        self.store.append_timeline(
            ...
            {"action_id": action.action_id, **reconciliation.to_dict()},
            idempotency_key=f"already-requested:{action.action_id}:{reconciliation.outcome}",
        )
        self._settle_run(run.run_id)
        ...
        for run in self.store.list_runs(patch_id=patch_id):
            for action in self.store.list_actions(run.run_id):
                ...
                if not reconciliation.already_requested:
                    continue
                key = f"ambiguous-reconciled:{action.action_id}:{reconciliation.outcome}"
                if any(
