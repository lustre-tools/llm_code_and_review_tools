...
MAX_FAILED_RUNS = 8
# 9999-12-31T23:59:59Z in milliseconds.  Jenkins timestamps and durations are
# millisecond integers, and ``datetime.fromtimestamp`` raises OverflowError --
# an ArithmeticError, which no caller catches -- above the platform's range.
# Bounding the field turns that into the same labelled snapshot failure as
...
def _nonnegative_int(value: Any, label: str, *, maximum: int = MAX_EPOCH_MS) -> int:
...
class JenkinsSnapshotClient:
    ...
    def fetch_failure_snapshot(
        ...
        *,
        ...
        completed_at = ""
        if timestamp > 0:
            # Each field is individually in range; their sum need not be, and
            # the conversion raises OverflowError -- an ArithmeticError that no
            # caller catches -- rather than ValueError.
            try:
                started_at = datetime.fromtimestamp(timestamp / 1000, tz=UTC).isoformat()
                completed_at = datetime.fromtimestamp(
                    (timestamp + duration) / 1000, tz=UTC
                ).isoformat()
            except (OSError, OverflowError, ValueError) as exc:
                raise JenkinsSnapshotError(
                    "Jenkins build timing is malformed"
                ) from exc
