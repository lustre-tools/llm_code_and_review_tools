...
def page(message=""):
    rows = "".join(
        ...
        f"<td><span class='badge'>{escape(p.get('status', 'Pending'))}</span></td><td>{escape(p.get('last_updated', '—'))}</td>"
        ...
        for p in PATCHES
    ) or "<tr><td colspan='4' class='empty'>No patches yet. Add a Gerrit change to start watching.</td></tr>"
...
<section class='card'><h2>Watched patches <small>({len(PATCHES)})</small></h2><table><thead><tr><th>Patch</th><th>Review status</th><th>Last updated</th><th></th></tr></thead><tbody>{rows}</tbody></table></section></main></body></html>"""
